...
var execute = module.exports.execute = function(client, environment, templates, machines, opts, callback) {
    ...
    _.each(machines, function(machine, name) {
        if (machine.remote) {
            ...
            roledefs[machine.template.name].push(machine.remote.primaryIp);
            ...
            if (machine.name !== 'puppet') {
                ...
                provisionGroups[machine.provisionGroup].hosts.push(machine.remote.primaryIp);
            }
        }
    });
    ...
    var pythonData = {
        ...
        'puppetHost': machines['puppet'].remote.primaryIp,
