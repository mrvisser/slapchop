...
var create = module.exports.create = function(client, machinesToCreate, keyname, callback, _failures) {
    ...
    // for testing add this to params...
    //                DryRun: true,
    ...
    if (machine.template.system.services) {
      _.each(machine.template.system.services, function(service) {
        params.TagSpecifications[0].Tags.push({ Key: service, Value: 'yes' });
      } );
    }
    // BUG this currently only allows one extra tag :(
    if (machine.template.system.tags) {
      _.each(_.keys(machine.template.system.tags), function(extratags) {
        params.TagSpecifications[0].Tags.push({ Key: extratags, Value: machine.template.system.tags[extratags] });
      });
    }
    // console.dir(JSON.stringify(params), {colors:true});
